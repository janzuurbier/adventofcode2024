	...
	for (auto it = points_per_freq.begin(); it != points_per_freq.end(); ++it) {
		...
		for(int i = 0; i < v.size(); i++)
			for (int j = i + 1; j < v.size(); j++) {
				...
				if (antenna_map.isInRange(v[i].first + a, v[i].second +  b)) {
					antinodes.insert(point(v[i].first + a, v[i].second + b));
				}
				if (antenna_map.isInRange(v[j].first -  a, v[j].second - b)) {
					antinodes.insert(point(v[j].first - a, v[j].second - b));
				}
